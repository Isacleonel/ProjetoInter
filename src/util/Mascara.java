...
public abstract class Mascara {
    ...
    public static void aceitaNum(final TextField textField) {
        textField.lengthProperty().addListener(new ChangeListener<Number>() {
            @Override
            public void changed(ObservableValue<? extends Number> observable, Number oldValue, Number newValue) {
    ...
    public static void aceitaNumPonto(final TextField textField) {
        textField.lengthProperty().addListener(new ChangeListener<Number>() {
            @Override
            public void changed(ObservableValue<? extends Number> observable, Number oldValue, Number newValue) {
                if (newValue.intValue() > oldValue.intValue()) {
    ...
    public static void colocaBarraData(DatePicker date) {
		date.setPromptText("DD/MM/AAAA");
		date.focusedProperty().addListener(new ChangeListener<Boolean>() {
			public void changed(ObservableValue<? extends Boolean> arg0, Boolean oldPropertyValue,
					Boolean newPropertyValue) {
				if (!newPropertyValue) {
					if (!date.getEditor().getText().isEmpty()) {
						if (!date.getEditor().getText().contains("/")) {
							DateTimeFormatter dateFormatter = DateTimeFormatter.ofPattern("ddMMyyyy");
							LocalDate aux = LocalDate.parse(date.getEditor().getText(), dateFormatter);
							date.setValue(aux);
						}
